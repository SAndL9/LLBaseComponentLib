...
#import "LLNumberButton.h"
@interface LLNumberButton ()<UITextFieldDelegate>
...
- (void)increase
{
    ...
    if (number < _maxNumber) {
        ...
        if (self.delegate && [self.delegate respondsToSelector:@selector(NumberButton:number:)]) {
            [self.delegate NumberButton:self number:_textField.text];
        }        
    }else{
